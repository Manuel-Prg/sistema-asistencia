        ...
        <div className="space-y-3 sm:space-y-4">
          {records.map((record, index) => {
            ...
            return (
              <div
                key={record.id}
                className="group relative overflow-hidden p-4 sm:p-5 bg-gradient-to-br from-gray-50 to-slate-50 rounded-xl border border-gray-200 hover:border-indigo-300 hover:shadow-lg transition-all duration-200"
              >
                {/* Decorative Elements */}
                <div className="absolute top-0 right-0 w-20 h-20 bg-indigo-100/30 rounded-full -mr-10 -mt-10 group-hover:bg-indigo-200/40 transition-colors" />
                ...
                <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                  ...
                  {/* Right Section - Hours */}
                  <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-1 pt-2 sm:pt-0 border-t sm:border-t-0 sm:border-l border-gray-200 sm:pl-4">
                    {!isActive ? (
                      <>
                        <div className="flex sm:flex-col items-baseline sm:items-end gap-1">
                          ...
                          <span className="text-sm sm:text-base text-gray-500 font-medium">horas</span>
                        </div>
                        {(record.hours_worked || 0) >= 4 ? (
                          <span className="text-xs text-emerald-600 font-medium">✓ Completo</span>
                        ) : (
                          <span className="text-xs text-amber-600 font-medium">⚠ Incompleto</span>
                        )}
                      </>
                    ) : (
                      <div className="flex items-center gap-2 px-3 py-2 bg-emerald-100 border border-emerald-300 rounded-full">
                        <div className="h-2 w-2 bg-emerald-500 rounded-full animate-pulse" />
                        <span className="text-sm font-semibold text-emerald-700">En curso</span>
                      </div>
                    )}
                  </div>
                </div>
